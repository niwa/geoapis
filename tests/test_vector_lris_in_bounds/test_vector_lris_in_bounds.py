...
class LrisVectorsTest(unittest.TestCase):
    """A class to test the basic vector.lris functionality by downloading files from the
    dataservice within a small region. The vector attributes are then compared against
    the expected.

    Tests run include (test_#### indicates the layer tested):
        * test_105112 - Test the specified layer features are correctly downloaded
          within the specified bbox
        * test_105112_no_geometry_name - Test the specified layer and bbox, but with no
          geometry_name given
        * test_104400 - Test the specified layer features are correctly downloaded
          within the specified bbox
        * test_104400_no_geometry_name - Test the specified layer and bbox, but with no
          geometry_name given
    ...
    # later tests
    NI_PASTURE = {
        "area": 717394.106703012,
        "geometryType": "Polygon",
        "length": 41471.63423421421,
        "columns": [
            "geometry",
            "cat",
            "area_m2",
        ...
        "uid": ["0900290150", "0900290514", "0900289928", "0900064270", "0900286983"],
    }
    LCDB_V5 = {
        "area": 89357299.57565206,
        "geometryType": "Polygon",
        "length": 578730.5152480144,
        "columns": [
            "geometry",
            "Name_2018",
            "Name_2012",
    ...
    def setUpClass(cls):
        ...
        file_path = pathlib.Path().cwd() / pathlib.Path(
            "tests/test_vector_lris_in_bounds/instruction.json"
        )
    ...
    def test_105112(self):
        ...
        features = self.runner.run(
            self.instructions["instructions"]["apis"]["lris"]["ni_pature_productivity"][
                "layers"
            ][0],
            self.instructions["instructions"]["apis"]["lris"]["ni_pature_productivity"][
                "geometry_name"
    ...
    def test_104400(self):
        ...
        features = self.runner.run(
            self.instructions["instructions"]["apis"]["lris"]["land_cover_database"][
                "layers"
            ][0],
            self.instructions["instructions"]["apis"]["lris"]["land_cover_database"][
                "geometry_name"
