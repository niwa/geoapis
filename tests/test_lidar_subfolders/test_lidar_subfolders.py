...
class OpenTopographyTestSubfolders(unittest.TestCase):
    ...
    def test_correct_datasets(self):
        ...
        self.assertEqual(len(list(self.cache_dir.glob('*/**'))), len(dataset_dirs), "There should be " +
        ...
        self.assertEqual(len([folder for folder in self.cache_dir.rglob("*") if folder.is_dir() and folder in
                              dataset_dirs]), len(dataset_dirs),
    ...
    def test_correct_files_downloaded(self):
        ...
        for key in self.FILE_SIZES.keys():
            ...
            self.assertTrue(numpy.all([file in dataset_files for file in dataset_dir.iterdir() if file.is_file()]),
    ...
    def test_correct_file_sizes(self):
        ...
        for key in self.FILE_SIZES.keys():
            ...
            self.assertTrue(numpy.all([dataset_file.stat().st_size == self.FILE_SIZES[key][dataset_file.name] for
                                       dataset_file in dataset_files]), "There is a miss-match between the size" +
