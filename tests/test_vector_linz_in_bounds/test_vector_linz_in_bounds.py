...
class LinzVectorsTest(unittest.TestCase):
    ...
    the dataservice within a small region. The vector attributes are then compared
    against the expected.

    Tests run include (test_#### indicates the layer tested):
        * test_51153 - Test the specified layer features are correctly downloaded within
          the specified bbox
        * test_51153_no_geometry_name - Test the specified layer and bbox, but with no
          geometry_name given
        * test_50448 - Test the specified layer features are correctly downloaded within
          the specified bbox
        * test_50448_no_geometry_name - Test the specified layer and bbox, but with no
          geometry_name given
        * test_50072 - Test the specified layer features are correctly downloaded within
          the specified bbox
        * test_50072_no_geometry_name - Test the specified layer and bbox, but with no
          geometry_name given
    ...
    LAND = {
        ...
        "name": ["South Island or Te Waipounamu"],
    }
    BATHYMETRY_CONTOURS = {
        "area": 0.0,
    ...
    def setUpClass(cls):
        ...
        file_path = pathlib.Path().cwd() / pathlib.Path(
            "tests/test_vector_linz_in_bounds/instruction.json"
        )
    ...
    def compare_to_benchmark(
        ...
        self.assertEqual(
            ...
            f"the first five entries `{list(features[column_name][0:5])}` differ from "
            f"the expected {benchmark[column_name]}",
        )
        self.assertEqual(
    ...
    def test_51153(self):
        ...
        features = self.runner.run(
            self.instructions["instructions"]["apis"]["linz"]["land"]["layers"][0],
    ...
    def test_50448(self):
        ...
        features = self.runner.run(
            self.instructions["instructions"]["apis"]["linz"]["bathymetry_contours"][
                "layers"
            ][0],
            self.instructions["instructions"]["apis"]["linz"]["bathymetry_contours"][
                "geometry_name"
    ...
    def test_50072(self):
        ...
        features = self.runner.run(
            self.instructions["instructions"]["apis"]["linz"]["chatham_contours"][
                "layers"
            ][0],
            self.instructions["instructions"]["apis"]["linz"]["chatham_contours"][
                "geometry_name"
        ...
        # check various shape attributes match those expected
        assert features is benchmark, (
            "No features should have been returned as the WFS search BBox does not "
            f"overlap with the {description} extents. Instead {features} was returned."
        )
